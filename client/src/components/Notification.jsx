      ...
      <div className="mt-5 ">
        { //chatData?.map((item, index) => (
          <div /* key={index} */ className="flex items-center leading-8 gap-5 border-b-1 border-color p-3">
            <img className="rounded-full h-10 w-10" src={avatar} alt='avatar' />
            <div>
              <p className="font-semibold dark:text-gray-200">Test Notification</p>
              <p className="text-gray-500 text-sm dark:text-gray-400"> Test Notification </p>
            </div>
          </div>
        //))
    }
